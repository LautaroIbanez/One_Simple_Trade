...
class MarketDataService:
    @staticmethod
    async def fetch_daily_ohlc(asset_id: str = "bitcoin", vs_currency: str = "usd", days: int = 730) -> tuple[pd.DataFrame, int, int]:
        """
        ...
        market_caps: List[Tuple[int, float]] = data.get("market_caps", [])
        volumes: List[Tuple[int, float]] = data.get("total_volumes", [])

        # Use close from prices and approximate OHLC as close-only candles (minimal but consistent)
        ...
        df["open"] = df["close"].shift(1).fillna(df["close"])  # first open == first close
